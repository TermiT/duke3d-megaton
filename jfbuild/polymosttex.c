...
static int pt_load_hightile(PTHead * pth)
{
	...
	for (texture = 0; texture < PTHPIC_SIZE; texture++) {
		...
		if (texture == 0) {
            ...
            assert(hicreplc[pth->picnum] != NULL);
            hicreplc[pth->picnum]->sizex = pth->pic[0]->sizx;
            hicreplc[pth->picnum]->sizey = pth->pic[0]->sizy;
//            if (pth->picnum == BOTTOMSTATUSBAR) {
//                extern void resizestatusbar(void);
//                resizestatusbar();
//            }
		}
...
static void ptm_fixtransparency(PTTexture * tex, int clamped)
{
	...
	for (y = doy; y >= 0; y--) {
		...
		for (x = dox; x >= 0; x--, wpptr--) {
            if (tilesizx[2536]==211 && tilesizy[2536]==55 && daxsiz2==211 && daysiz2==55)  // CHAINGUN 2536
            {
                if (doy>=3 && y==2 && x>=78 && x<=157)
                {
                    const coltype *onedown = &tex->pic[(y+1)*daxsiz2+x];
                    //printf("dsasd %ld\n",(y+1)*daxsiz2+x);
