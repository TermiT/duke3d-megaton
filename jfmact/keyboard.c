...
{
    static char const *mbuttonnames[9] = {
            "$mouse0", "$mouse1", "$mouse2", "$mouse3", "$mouse4", "$mouse5", "$mouse6", "$mouse7", "$mouse8",
    };
	unsigned s;
    ...
    if ( scancode > 0 && scancode & 0x10000) {
        int mbutton = scancode & 0xFF;
        if (mbutton < 10) {
            return mbuttonnames[mbutton];
        }
    }
...
{
	if (keypressed) KB_LastScan = (kb_scancode)scancode;
}
